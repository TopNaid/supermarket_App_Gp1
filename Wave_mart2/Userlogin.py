...
class Login:
    ...
    # WIDGET FUNCTION
    def obj(self):
        self.loginframe = Canvas(self.loginw, bg="#f2f2f2", height=400, width=300, highlightthickness=2)
        self.loginw.bind('<Return>', self.checkuser)
        self.loginframe.place(x=85, y=95)
        self.toplabel = Label(self.loginframe, fg="black", bg="#f2f2f2", anchor="center", text="Login",
                              font="Montserrat 30 normal")
        self.toplabel.place(x=95, y=30)
        self.us = Entry(self.loginframe, width=20, bg="#ffffff", textvariable=self.username, border=0, justify='center',
                        font="Roboto 14 ")
        self.us.place(x=35, y=145, height=40)
        self.pa = Entry(self.loginframe, width=20, bg="#ffffff", textvariable=self.password, border=0, justify='center',
                        font="Roboto 14 ")
        self.pa.place(x=35, y=190, height=40)
        self.us.bind('<Button-1>', self.onclick)
        self.pa.bind('<Button-1>', self.onclick1)
        self.signin = Button(self.loginframe, width=20, text="Sign in", bg="#9933ff", fg="white",
